...
import {
    ...
    TouchableOpacity, TextInput, Platform, ScrollView, // Adicionado ScrollView para filtros
    Alert
} from 'react-native';
...
import {
    ...
    doc, deleteDoc, // Funções necessárias
    Timestamp // Para tipos
...
export default function AllTransactionsScreen() {
  ...
  useEffect(() => {
    ...
    console.log("AllTransactionsScreen: Setting up listener for group:", groupId);
    ...
    const unsubscribe = onSnapshot(transQuery, (snapshot) => {
      ...
      snapshot.forEach(doc => { /* ... preenche fetched como antes ... */
          const data = doc.data();
      ...
      console.log(`AllTransactionsScreen: Fetched ${fetched.length} transactions.`);
      setRawTransactions(fetched); // Guarda transações brutas ordenadas pelo Firestore
      setIsLoading(false);
    }, (error) => { /* ... tratamento erro ... */ setIsLoading(false); });
  ...
  const filteredTransactions = useMemo(() => {
    console.log("AllTransactionsScreen: Recalculating filtered transactions...");
    ...
    // Não precisa re-ordenar aqui se a query do Firestore já ordena por data
    return itemsToFilter;

  }, [rawTransactions, selectedCategories, transactionTypeFilter, transactionSearchQuery]);
  // ---------------------------------------

  // --- Handlers ---
  const handleCategoryFilterChange = (category: string) => { /* ... como em HomeScreen ... */
      setSelectedCategories(prev => category === 'Todas' ? [] : (prev.includes(category) ? prev.filter(c => c !== category) : [...prev, category]));
  };
   const handleAddCategory = useCallback(async (newCategory: string) => { /* ... como em HomeScreen ... */ }, [groupId, groupData]); // Precisa de groupData

  // Handlers Modais e Ações de Transação (Idênticos aos de HomeScreen)
  const handleTransactionPress = (transaction: Transaction) => {
      setSelectedTransaction(transaction);
      setIsDetailModalVisible(true);
  };
  const handleCloseDetailModal = () => { setIsDetailModalVisible(false); setSelectedTransaction(null); };
  const closeAddEditModal = () => { setIsAddModalVisible(false); setEditingTransaction(null); };
  const handleEditTransaction = (transaction: Transaction) => {
   ...
   const handleDeleteTransaction = async (transactionId: string) => {
      if (!groupId) return;
      // Confirmação está no Modal de Detalhes, aqui só executa
      console.log("Deleting transaction:", transactionId);
      const transDocRef = doc(db, "groups", groupId, "transactions", transactionId);
      try {
          await deleteDoc(transDocRef);
          handleCloseDetailModal(); // Fecha o modal que originou a ação
  ...
  // --- Componente Cabeçalho da Lista (Filtros) ---
  const ListHeader = () => (
           ...
           <View style={styles.typeFilterContainer}>
            ...
            <View style={styles.searchContainer}>
                ...
                {transactionSearchQuery.length > 0 && Platform.OS !== 'ios' && (
                    <TouchableOpacity onPress={() => setTransactionSearchQuery('')} style={styles.clearButton}>
            ...
            <View style={styles.filterContainer}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterChips}>
                   <TouchableOpacity /* Botão Todas Cat. */ onPress={() => handleCategoryFilterChange('Todas')} style={[styles.chip, selectedCategories.length === 0 ? styles.chipSelected : styles.chipIdle ]} >
                       <Text style={[styles.chipText, selectedCategories.length === 0 ? styles.chipTextSelected : styles.chipTextIdle ]}>Todas Cat.</Text>
                   </TouchableOpacity>
                   {(groupData?.categories || []).sort((a,b)=>a.localeCompare(b)).map((cat: string) => (
                       <TouchableOpacity key={cat} onPress={() => handleCategoryFilterChange(cat)} style={[styles.chip, selectedCategories.includes(cat) ? styles.chipSelected : styles.chipIdle ]}>
                           <Text style={[styles.chipText, selectedCategories.includes(cat) ? styles.chipTextSelected : styles.chipTextIdle ]}>{cat}</Text>
                       </TouchableOpacity>
                   ))}
               </ScrollView>
           </View>
  ...
  return (
    <View style={styles.container}>
        {/* Configura o header da Stack */}
        <Stack.Screen options={{ title: 'Histórico Completo', headerShown: true }} />

        <FlatList
            data={filteredTransactions} // Usa a lista FILTRADA
            renderItem={({item}) => ( // Usa o componente de item reutilizável
                <TransactionListItem item={item} onPress={handleTransactionPress} />
            )}
            keyExtractor={(item) => item.id}
            contentContainerStyle={styles.listContent}
            ListHeaderComponent={ListHeader} // Adiciona os filtros no cabeçalho da lista
            ListEmptyComponent={ // Mostra se a lista (filtrada ou não) está vazia
                !isLoading ? ( // Só mostra se não estiver carregando
                    <View style={styles.centered}>
                        <Ionicons name="receipt-outline" size={60} color={colors.textSecondary} style={{marginTop: 30}} />
                        <Text style={styles.emptyTitle}>{transactionSearchQuery || selectedCategories.length > 0 || transactionTypeFilter !== 'all' ? 'Nenhuma Transação Encontrada' : 'Sem Transações'}</Text>
                        <Text style={styles.emptyText}>
                            {transactionSearchQuery || selectedCategories.length > 0 || transactionTypeFilter !== 'all'
                                ? 'Nenhuma transação corresponde aos filtros aplicados.'
                                : 'Registre sua primeira entrada ou saída.'}
                        </Text>
                    </View>
                ) : null // Não mostra nada se estiver carregando
            }
            // Indicador de loading no final (útil para paginação futura)
            ListFooterComponent={isLoading ? <ActivityIndicator style={{margin: 20}} color={colors.primary}/> : null}
        />

       {/* Modais (Renderizados mas invisíveis até serem ativados) */}
       <AddTransactionModal
...
const getStyles = (colors: any) => StyleSheet.create({
  ...
  listContent: { paddingHorizontal: 15, paddingTop: 0, paddingBottom: 20 }, // paddingTop 0 pois header tem padding
  emptyTitle: { fontSize: 20, fontWeight: 'bold', color: colors.textPrimary, marginTop: 15, marginBottom: 5, textAlign: 'center' },
  emptyText: { fontSize: 16, color: colors.textSecondary, textAlign: 'center', paddingHorizontal: 20 },
  filtersSection: { // Container para todos os filtros
    paddingBottom: 10,
    marginBottom: 5,
    borderBottomWidth: 1,
    borderBottomColor: colors.border,
  },
  // Copiado de HomeScreen (ajustar margens/padding se necessário)
  ...
  typeFilterButtonSegmented: { borderColor: colors.primary, borderLeftWidth: 0.5, borderRightWidth: 0.5 },
  ...
  filterContainer: { marginBottom: 0 }, // Container dos chips sem margem inferior extra
  filterTitle: { fontSize: 14, color: colors.textSecondary, marginBottom: 10 },
